...
function loadPokemon(pokemonData) {
    $.getJSON(getWorksheetUrl(spreadsheetId, worksheetId), function(spreadsheet) {
        ...
        $(rows).each(function() {
            ...
            pokemon.genderRatio = pokemonData[slug].gr;
        ...
        for (let i = 0; i < EGG_GROUPS.length; i++) {
            $dropdown.append(createCheckbox('egg-group', EGG_GROUPS[i], slugify(EGG_GROUPS[i])));
        }
...
$(document).ready(function() {
    $.getJSON(getWorksheetUrl(spreadsheetId, 1), function(data) {
        ...
        if (entry.gsx$ingamename) {
            friendCode = tryGetValue(entry, ['friendcode'], friendCode);
            inGameName = tryGetValue(entry, ['ingamename'], inGameName);
            contactUrl = tryGetValue(entry, ['contacturl'], contactUrl);
            worksheetId = 2;
